...
#update tls
ansible-playbook -i ../hosts ../01_install.yml -uroot -t tls || exit 1
echo "play tls OK >>>"

#etcd restarted
ansible -i ../hosts etcd   -m shell   -a 'rm -rf {{ etcd_work_dir }}/ssl/*.pem' || exit 1
ansible-playbook -i ../hosts ../01_install.yml -uroot -t etcd_ssl_dist || exit 1
...
ansible-playbook -i ../hosts ../01_install.yml -uroot -t master_k8s_dist,master_etcd_dist,node_k8s_dist  || exit 1
...
ansible-playbook -i ../hosts ../01_install.yml -uroot -t addons || exit 1
echo "update tls OK <<<"